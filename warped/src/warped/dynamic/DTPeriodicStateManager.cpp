...
void DTPeriodicStateManager::saveState(const VTime& currentTime,
	...
	//				<< eventNumber << endl;
			DTStateManagerImplementationBase::getStateQueueLock(threadID,
					simObjectID);
			...
			DTStateManagerImplementationBase::releaseStateQueueLock(threadID,
					simObjectID);
		}
...
const unsigned int DTPeriodicStateManager::getEventIdForRollback(int threadId,
		int objId) {
	//    const EventId *ret;
	...
	//    return ret;
	return DTStateManagerImplementationBase::getEventIdForRollback(threadId,
			objId);

}
void DTPeriodicStateManager::updateStateWhileCoastForward(
