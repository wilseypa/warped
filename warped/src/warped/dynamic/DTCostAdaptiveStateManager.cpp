...
DTCostAdaptiveStateManager::DTCostAdaptiveStateManager(
		DTTimeWarpSimulationManager *simMgr) :
	DTAdaptiveStateManagerBase(simMgr) {
...
void DTCostAdaptiveStateManager::calculatePeriod(SimulationObject *object) {
...
void DTCostAdaptiveStateManager::coastForwardTiming(unsigned int id,
		double coastforwardtime) {
	CoastForwardTimeWeighted[id].update(coastforwardtime);
}

double DTCostAdaptiveStateManager::getCoastForwardTime(unsigned int id) {
	return (CoastForwardTimeWeighted[id].getData());
}

void DTCostAdaptiveStateManager::saveState(const VTime& currentTime,
	...
	DTAdaptiveStateManagerBase::saveState(currentTime, object, threadId);
}
