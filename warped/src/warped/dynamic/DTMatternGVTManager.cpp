...
DTMatternGVTManager::DTMatternGVTManager(DTTimeWarpSimulationManager *simMgr,
		unsigned int period) :
...
const VTime *DTMatternGVTManager::getEarliestEventTime(
	...
	if (mySimulationManager->getOutputMgrType() == LAZYMGR
		...
		DTLazyOutputManager *lmgr =
				dynamic_cast<DTLazyOutputManager*>(mySimulationManager->getOutputManagerNew());
...
void DTMatternGVTManager::receiveKernelMessage(KernelMessage *msg) {
	ASSERT(msg != NULL);
...
void DTMatternGVTManager::sendPendingGVTToken() {
