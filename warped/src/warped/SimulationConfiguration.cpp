...
bool
SimulationConfiguration::objectQueueTypeIs( const string &testValue ) const {
...
const string
SimulationConfiguration::getObjectQueueType() const {
  return _impl->getObjectQueueType();
}

bool SimulationConfiguration::getObjectQueueNumberOfBuckets(unsigned int &numberOfBuckets) const {
	return _impl->getObjectQueueNumberOfBuckets(numberOfBuckets);
}

bool SimulationConfiguration::getObjectQueueBucketWidth(unsigned int &bucketWidth) const {
...
bool
SimulationConfiguration::optFossilCollManagerTypeIs( const string &testValue ) const {
  return _impl->optFossilCollManagerTypeIs( testValue );
}

const string
SimulationConfiguration::getOptFossilCollManagerType() const {
...
bool
SimulationConfiguration::antiMessagesIs( const string &testValue ) const {
  return _impl->antiMessagesIs( testValue );
}

const string
SimulationConfiguration::getAntiMessagesType() const {
...
bool
SimulationConfiguration::getOptFossilCollPeriod( unsigned int &period ){
  return _impl->getOptFossilCollPeriod( period );
}

bool
SimulationConfiguration::getOptFossilCollMinSamples( unsigned int &minSamples ){
  return _impl->getOptFossilCollMinSamples( minSamples );
}

bool
SimulationConfiguration::getOptFossilCollMaxSamples( unsigned int &maxSamples ){
  return _impl->getOptFossilCollMaxSamples( maxSamples );
}

bool
SimulationConfiguration::getOptFossilCollDefaultLength( unsigned int &defaultLength ){
...
bool SimulationConfiguration::getWorkerThreadCount(unsigned int &workerThreadCount) const {
...
SimulationConfiguration::Implementation::getEventListScope() const {
  ...
  else if( simType == "TIMEWARP" || simType == "THREADEDWARP" || simType == "DTTIMEWARP" ){
    const ConfigurationScope *twScope = getTimeWarpScope();
    if( twScope != 0 ){
...
SimulationConfiguration::Implementation::communicationManagerIs( const string &testValue ) const {
  return ( stringToUpper(testValue) == stringToUpper(getCommunicationManagerType()) );
}
...
SimulationConfiguration::Implementation::getEventListType() const {
  string retval = "(none)";
  if( getEventListScope() != 0 ){
    const ConfigurationChoice *type = getEventListScope()->findChoice( getTypeName() );
    if( type != 0 ){
      retval = stringToUpper(type->getStringValue());
    }
  }
...
SimulationConfiguration::Implementation::getEventListOrganization() const {
  string retval = "(none)";
  if( getEventListScope() != 0 ){
    const ConfigurationChoice *Organization = getEventListScope()->findChoice( getOrganizationName() );
    if( Organization != 0 ){
...
SimulationConfiguration::Implementation::eventListOrganizationIs( const string &testValue ) const {
  return ( stringToUpper(testValue) == stringToUpper(getEventListOrganization()) );
}

const string
SimulationConfiguration::Implementation::getObjectQueueType() const {
  string retval = "(none)";
  if( getObjectQueueScope() != 0 ){
    const ConfigurationChoice *type = getObjectQueueScope()->findChoice( getTypeName() );
    if( type != 0 ){
    	retval = stringToUpper(type->getStringValue());
    }
  }
...
bool
SimulationConfiguration::Implementation::objectQueueTypeIs( const string &testValue ) const {
  return ( stringToUpper(testValue) == stringToUpper(getObjectQueueType()) );
}
bool SimulationConfiguration::Implementation::getObjectQueueNumberOfBuckets(unsigned int &numberOfBuckets) const
{
	bool retval = false;
...
bool SimulationConfiguration::Implementation::getObjectQueueBucketWidth(unsigned int &bucketWidth) const
{
	bool retval = false;
	const ConfigurationScope *scope = getObjectQueueScope();
	if( scope != 0 ){
...
const string
SimulationConfiguration::Implementation::getOptFossilCollManagerType() const {
  string retval = "(none)";
  if( getOptFossilCollManagerScope() != 0 ){
    const ConfigurationChoice *type = getOptFossilCollManagerScope()->findChoice( "Type" );
    if( type != 0 ){
      retval = stringToUpper(type->getStringValue());
    }
  }
...
bool
SimulationConfiguration::Implementation::optFossilCollManagerTypeIs( const string &testValue ) const {
  return ( stringToUpper(testValue) == stringToUpper(getOptFossilCollManagerType()) );
}
...
const string
SimulationConfiguration::Implementation::getAntiMessagesType() const {
  string retval = "(none)";
  if( getOutputManagerScope() != 0 ){
    const ConfigurationChoice *type = getOutputManagerScope()->findChoice( "AntiMessages" );
    if( type != 0 ){
      retval = stringToUpper(type->getStringValue());
    }
  }
...
bool
SimulationConfiguration::Implementation::antiMessagesIs( const string &testValue ) const {
...
bool SimulationConfiguration::Implementation::getWorkerThreadCount(unsigned int &workerThreadCount) const{
  bool retval = false;
...
bool
SimulationConfiguration::Implementation::getOptFossilCollPeriod( unsigned int &period ){
  bool retval = false;
  if( getOptFossilCollManagerScope() != 0 ){
    if( getOptFossilCollManagerScope()->findChoice( "CheckpointTime" ) != 0 ){
      period = getOptFossilCollManagerScope()->findChoice( "CheckpointTime" )->getIntValue();
      retval = true;
    }
  }
  return retval;
...
bool
SimulationConfiguration::Implementation::getOptFossilCollMinSamples( unsigned int &minSamples ){
  bool retval = false;
  if( getOptFossilCollManagerScope() != 0 ){
    if( getOptFossilCollManagerScope()->findChoice( "MinimumSamples" ) != 0 ){
      minSamples = getOptFossilCollManagerScope()->findChoice( "MinimumSamples" )->getIntValue();
      retval = true;
    }
  }
  return retval;
...
bool
SimulationConfiguration::Implementation::getOptFossilCollMaxSamples( unsigned int &maxSamples ){
  bool retval = false;
  if( getOptFossilCollManagerScope() != 0 ){
    if( getOptFossilCollManagerScope()->findChoice( "MaximumSamples" ) != 0 ){
      maxSamples = getOptFossilCollManagerScope()->findChoice( "MaximumSamples" )->getIntValue();
      retval = true;
    }
  }
  return retval;
...
bool
SimulationConfiguration::Implementation::getOptFossilCollDefaultLength( unsigned int &defaultLength ){
  bool retval = false;
  if( getOptFossilCollManagerScope() != 0 ){
    if( getOptFossilCollManagerScope()->findChoice( "DefaultLength" ) != 0 ){
      defaultLength = getOptFossilCollManagerScope()->findChoice( "DefaultLength" )->getIntValue();
      retval = true;
    }
  }
  return retval;
...
bool
SimulationConfiguration::Implementation::getOptFossilCollRiskFactor( double &riskFactor ){
  bool retval = false;
  if( getOptFossilCollManagerScope() != 0 ){
    if( getOptFossilCollManagerScope()->findChoice( "AcceptableRisk" ) != 0 ){
      riskFactor = getOptFossilCollManagerScope()->getDoubleValue( "AcceptableRisk" );
      retval = true;
    }
  }
  return retval;
