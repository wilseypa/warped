...
Iscas85Application::initialize( vector<string> &arguments ){
   string path="circuitsimulationmodels/iscas85/iscas85Sim/c3540/";
	//cout<<"arguments[4] is "<<arguments[4]<<endl;
	//cout<<"arguments[2] is "<<arguments[2]<<endl;
	//arguments[4]=path+arguments[4];
	arguments[2]=path+arguments[2];
  ...
  /*
	if ( testCaseFileName.empty() ){
		std::cerr<< "A testcase file must be specified using -test"<<std::endl;
		abort();
	}
  // how to deal with testCaseFile
	string command ="/bin/bash "+testCaseFileName; 
...
Iscas85Application::getArgumentParser(){
	static ArgumentParser::ArgRecord args[] = {
	 ...
	 // {"-test","testcase file name",&testCaseFileName,ArgumentParser::STRING,false },//	
		{"","",0 }
