...
FileReaderWriter::initialize(){
	//outputHandle = getObjectHandle(desGateName);
	//cout<<"the destination object name is " <<desGateName<<endl;
  ...
  if("I"==IO){
  //send the first line (bit) of the file to every object the file connects
    ...
    ostringstream bitstring;
      ...
      //cout<<"numLinesProcessed value is:"<<state->numLinesProcessed<<endl;
