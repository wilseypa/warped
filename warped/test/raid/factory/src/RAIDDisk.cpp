  ...
  while(true == haveMoreEvents()) {
    ...
    if ( recvEvent != NULL ) {
      ...
      *tmpEvent = *recvEvent;
      ...
      nextCylinder = ( tmpEvent->getStartStripe() / (sectorsPerTrack*tracksPerCyl) ) % numCylinders;
      tmpEvent->setStartCylinder(nextCylinder);
      tmpEvent->setStartSector(tmpEvent->getStartStripe() % sectorsPerTrack);
...
void
RAIDDisk::deallocateState(const State* state){
  delete state;
} // End of deallocateState()

void
RAIDDisk::reclaimEvent(const Event *event){
   delete event;
} // End of reclaimEvent()

void
RAIDDisk::reportError(const string&, SEVERITY){
} // End of reportError()
