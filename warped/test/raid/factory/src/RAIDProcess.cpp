...
RAIDProcess::RAIDProcess(string &myName, string &outName, int maxdisks, 
  ...
  objectName(myName), outputName(outName), maxDisks(maxdisks),
   maxRequests(maxrequests), firstDiskId(0), diskType(disk), seed(initSeed) {
} // End of RAIDProcess()
...
RAIDProcess::initialize() {
  ...
  RAIDRequest* newRequest = new RAIDRequest(sendTime,
                                            sendTime + 1,
                                            this,
                                            this);
...
void
RAIDProcess::finalize(){
  RAIDProcessState *myState = dynamic_cast<RAIDProcessState *>(getState());
  cout << getName() << " Finished:\n"
       << "Number of Requests: " << myState->getNumRequests()
       << " Reads: " << myState->getNumReads()
       << " Writes " << myState->getNumWrites() << "\n";
} // End of finalize().
...
/*#if WHATISTHEPURPOSEOFTHIS
	...
	//  << " in diskRequests count\n";
      ...
      if ( (recvEvent->getRead() == false) && myState->getParitySize() == -1) { 
	...
	myState->set_size(myState->get_size() - 1);
      ...
      if (recvEvent->getRead() == true) {
        ...
        if ( (myState->get_size() == 0) && (myState->getParitySize() == -1 )) {
	...
	// event is a write
	// see if the operation is complete
	if ( (myState->get_size() == 0) && (myState->getParitySize() == 0)) {
	  myState->setDiskOperationPending(false);
...
RAIDProcess::newMsg() {
  ...
  if ( (newRequest->getStartStripe() + newRequest->getSizeRead()) >= totalStripeSize) {
...
void
RAIDProcess::deallocateState(const State* state){
  delete state;
} // End of deallocateState()

void
RAIDProcess::reclaimEvent(const Event *event){
  delete event;
} // End of reclaimEvent()

void
RAIDProcess::reportError(const string&, SEVERITY){
} // End of reportError()
