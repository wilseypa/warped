...
private:
    ...
    unsigned int recurse_rung() {
        ...
        do {
            ...
            if ((!isFindBucket) && (bucketIndex < numBucket[nRung-1])
                    && (nBucket[nRung-1][bucketIndex] >= THRESHOLD)) {
                ...
                for (tempIndex = 0; tempIndex < nBucket[nRung-2][bucketIndex]; tempIndex++) {
                    ...
                    rung[nRung-1][newBucketIndex][ nBucket[nRung-1][newBucketIndex]++ ] =
                        rung[nRung-2][bucketIndex][tempIndex];
                }
