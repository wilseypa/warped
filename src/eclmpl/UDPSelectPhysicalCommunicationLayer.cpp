...
UDPSelectPhysicalCommunicationLayer::probeNetwork() {
    ...
    for (unsigned int i = 0; i < physicalSize; i++) {
        ...
        if (static_cast<UDPConnectionInterface*>(connInterface)->recvSocket[i]->wFD_ISSET(
...
UDPSelectPhysicalCommunicationLayer::initializeCommunicationLayerAttributes() {
    ...
    for (unsigned int i = 0; i < physicalSize; i++) {
        ...
        if (i != physicalId) {
            FD_SET(static_cast<UDPConnectionInterface*>(connInterface)->recvSocket[i]->getSocketFd(),
                   &selectListenSet);
        }
    }
...
UDPSelectPhysicalCommunicationLayer::UDP_SynchronizeMaster(const eclmplUnreliableNetworkMessageType&
    ...
    ECLMPL_DEBUG_FILE(outfile << physicalId << ": " << hostName << " SYNCHING on \"" << type << "\"" <<
                      endl;)
    ...
    ECLMPL_DEBUG_FILE(outfile << "                                                 " << physicalId <<
                      ": " << hostName
