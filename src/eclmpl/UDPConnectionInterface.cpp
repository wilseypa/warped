...
UDPConnectionInterface::createSocketPtrVector(std::vector<eclmplSocket*>& socketVector) {
    ...
    for (unsigned int i = 0; i < numberOfConnections; i++) {
        ...
        ASSERT(socketVector[i]->wSetsockopt(SOL_SOCKET, SO_SNDBUF, (int*)&sendSocketBufferSize,
                                            sizeof(int)) != -1);
        ASSERT(socketVector[i]->wSetsockopt(SOL_SOCKET, SO_RCVBUF, (int*)&recvSocketBufferSize,
                                            sizeof(int)) != -1);
    }
