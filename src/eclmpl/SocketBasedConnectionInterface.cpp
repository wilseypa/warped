...
SocketBasedConnectionInterface::setDefaultSocketOptions(const unsigned int& id) {
    ...
    ASSERT(socket[id]->wSetsockopt(SOL_SOCKET, SO_SNDBUF, (int*)&sendSocketBufferSize,
                                   sizeof(int)) != -1);
    ASSERT(socket[id]->wSetsockopt(SOL_SOCKET, SO_RCVBUF, (int*)&recvSocketBufferSize,
                                   sizeof(int)) != -1);
...
#if 0
    std::cerr << connectionId << ": _____recv________ mtu="<<mtu<<", msgSize="<<msgSize<<", rcvFr="
              <<sourceId<<"_____________" << std::endl;
#endif
