...
public:
    ...
    /**@name Public Class Methods of OutputEvents. */
    //@{

    /** Constructor
        @param simMgr A handle to the simulation manager.
    */
    OutputEvents(TimeWarpSimulationManager* simMgr);
    ...
    */
    ...
    /** Returns the events sent at or after the search time and removes them from
        ...
        @return vector The vector of the events sent at or after the search time.
    */
    std::vector<const Event*>* getEventsSentAtOrAfterAndRemove(const VTime& searchTime);
    ...
    */
    void fossilCollect(const VTime& gCollectTime);
    ...
    /** Reclaim any events with a send time less than the
        ...
        @param gCollectTime The time to fossil collect at.
    */
    void fossilCollect(int gCollectTime);

    /** Reclaims only a single event.

        @param toRemove The event to reclaim.
    */
    void fossilCollectEvent(const Event* toRemove);
    ...
    */
    const Event* getOldestEvent(unsigned int size);
    ...
    */
    void remove(const std::vector<const Event*>& toRemove);
    ...
    */
    ...
    //@} // End of Public Class Methods of OutputEvents.

private:

    /**@name Private Class Methods of OutputEvents. */
    //@{
