    ...
    while (time >= nextCheckpointTime[id]) {
        ...
        if (it != checkpointedStates.end()) {
            ...
            for (int i = 0; i < mySimManager->getNumberOfSimulationObjects(); i++) {
                ...
                mySimManager->getOutputManager()->saveOutputCheckpoint(&ckptFile, ObjectID(i,simMgrID),
                                                                       lastCheckpointTime[i]);
...
OptFossilCollManager::restoreCheckpoint(unsigned int restoredTime) {
    ...
    for (int i = 0; i < mySimManager->getNumberOfSimulationObjects(); i++) {
        ...
        stringstream filename;
        filename << ckptFilePath << "LP" << mySimManager->getSimulationManagerID() << "." << restoredTime <<
                 "." << i;
        ifstream ckFile;
